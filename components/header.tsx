...
export default function Header() {
  ...
  return (
    <nav className="z-10 mt-5 w-full border-b border-black/5 dark:border-white/5 lg:border-transparent">
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-6 py-3 md:gap-0 md:py-4">
          <div className="relative z-20 flex w-full justify-between md:px-0 lg:w-max">
            ...
            <Link href="/" aria-label="logo" className="flex items-center space-x-2">
              <Image unoptimized fetchPriority="high" loading="lazy" src="./logo.png" alt="Stability Nexus Logo" 
                height={50}
                width={50}
              />
            ...
            <div className="relative flex max-h-10 items-center lg:hidden">
              <button aria-label="menu" id="menu" className="relative -mr-6 p-6" onClick={toggleMenu}>
          ...
          {/* Large Screen Menu */}
          <div id="navlinks" className="invisible absolute left-0 top-full z-20 w-full origin-top-right translate-y-1 scale-90 flex-col flex-wrap justify-end gap-6 rounded-3xl border border-gray-100 bg-white p-8 opacity-0 shadow-2xl shadow-gray-600/10 transition-all duration-300 dark:border-gray-700 dark:bg-gray-800 dark:shadow-none lg:visible lg:relative lg:flex lg:w-7/12 lg:translate-y-0 lg:scale-100 lg:flex-row lg:items-center lg:gap-0 lg:border-none lg:bg-transparent lg:p-0 lg:opacity-100 lg:shadow-none">
      ...
      {/* Small Screen Menu */}
      <div className={`lg:hidden flex flex-col items-end ${isOpen ? "block" : "hidden"}`}>
        {links.map((link, index) => (
          <Link
            href={link.to}
            onClick={toggleMenu}
            className={`block py-2 font-medium text-right w-full transition hover:text-primary dark:hover:text-white md:px-4 ${path === link.to && "font-semibold underline underline-offset-4"}`}
          >
            <span>{link.label}</span>
          </Link>
        ))}
